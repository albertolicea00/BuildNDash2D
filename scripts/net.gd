...
## the host then verifies every client request (cooldowns, placement bounds)
## instead of trusting the remote peer. The host also owns the level
## timeline, the scroll speed, triggers and moving platforms.
extends Node

enum Mode { LOCAL, HOST, CLIENT }

## Each game uses its own port so two games can run on the same LAN
## without clashing. BuildNDash2D = 7803.
const PORT := 7803
